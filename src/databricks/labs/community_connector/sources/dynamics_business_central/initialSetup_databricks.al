...
* Adapted for Databricks Lakeflow connector (pull-based delete tracking).
*/

table 90100 "Deleted Record"
{
    ...
    fields
    {
        ...
        field(3; companyId; Text[200])
        {
            Caption = 'Company ID';
        }
        field(4; deletedSystemId; Text[200])
        {
            Caption = 'Deleted System ID';
        }
        field(5; recordPosition; Text[500])
        {
            Caption = 'Record Position';
        }
        field(6; deletedAt; DateTime)
        {
            Caption = 'Deleted At';
        }
        field(7; systemModifiedAtOriginal; DateTime)
        {
            Caption = 'Original System Modified At';
        }
    }
    ...
    keys
    {
        ...
        key(TableCompany; tableId, companyId, deletedAt)
        {
        }
    }
...
page 90100 "Deleted Records"
{
    PageType = API;
    ApplicationArea = All;
    SourceTable = "Deleted Record";
    APIVersion = 'v1.0';
    APIPublisher = 'databricks';
    APIGroup = 'deleteTracking';
    EntityName = 'deletedRecord';
    EntitySetName = 'deletedRecords';
    DelayedInsert = false;
    Editable = false;
    ...
    layout
    {
        area(Content)
        {
            repeater(General)
            {
                ...
                field(deletedSystemId; Rec.deletedSystemId)
                {
                    Caption = 'Deleted System ID';
                }
                field(recordPosition; Rec.recordPosition)
                {
                    Caption = 'Record Position';
                }
                field(deletedAt; Rec.deletedAt)
                {
                    Caption = 'Deleted At';
                }
                field(systemModifiedAtOriginal; Rec.systemModifiedAtOriginal)
                {
                    Caption = 'Original System Modified At';
                }
...
table 90101 "Delete Tracker"
{
    ...
    fields
    {
        ...
        field(4; recordsDeleted; BigInteger)
        {
    ...
    keys
    {
        ...
        key(TableCompany; companyId, tableId)
        {
        }
    }
...
page 90101 "Delete Trackers"
{
    PageType = API;
    ApplicationArea = All;
    SourceTable = "Delete Tracker";
    APIVersion = 'v1.0';
    APIPublisher = 'databricks';
    APIGroup = 'deleteTracking';
    EntityName = 'deleteTracker';
    EntitySetName = 'deleteTrackers';
    DelayedInsert = true;
    ODataKeyFields = SystemId;
    Extensible = false;
...
permissionset 90100 CONNECTOR_SETUP
{
    Assignable = true;
    Permissions =
        tabledata "Deleted Record" = RIMD,
        tabledata "Delete Tracker" = RIMD,
        table "Deleted Record" = X,
        table "Delete Tracker" = X,
        codeunit DeleteEventHandler = X;
}

codeunit 90100 DeleteEventHandler
{
    ...
    var
        deletedRecord: Record "Deleted Record";
        tracker: Record "Delete Tracker";
    ...
    begin
        ...
        tracker.SetCurrentKey(tableId);
        tracker.SetFilter(tableId, Format(RecRef.Number()));

        if (tracker.FindFirst()) then begin
            if (tracker.companyId = 'ALL') then begin
        ...
        if (not recordOfSharedTable) then begin
            ...
            tracker.SetCurrentKey(companyId, tableId);
            tracker.SetFilter(tableId, Format(RecRef.Number()));
            tracker.SetFilter(companyId, Format(company.Id).ToLower().Replace('{', '').Replace('}', ''));
        end;
        ...
        if tracker.FindSet() then begin
            ...
            deletedRecord.Init();
            deletedRecord.tableId := RecRef.Number();
            deletedRecord.companyId := Format(company.Id).ToLower().Replace('{', '').Replace('}', '');
            deletedRecord.deletedSystemId := Format(RecRef.Field(2000000000).Value).ToLower().Replace('{', '').Replace('}', '');
            deletedRecord.recordPosition := CopyStr(RecRef.GetPosition(), 1, 500);
            deletedRecord.systemModifiedAtOriginal := systemModifiedAt;
            deletedRecord.Insert(true);

            repeat
                tracker.recordsDeleted := tracker.recordsDeleted + 1;
                tracker.Modify();
...
/*
* Pages for metadata and standard tables, exposing all supported columns.
*/
